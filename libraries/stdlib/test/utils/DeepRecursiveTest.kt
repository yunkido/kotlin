...
class DeepRecursiveTest {
    ...
    fun testDeepTreeOddEvenNodesMutual() {
        val n = 1_000_000
        val dt = deepTree(n)
        assertEquals(n / 2, mutualRecursion.even(dt))
        assertEquals(n / 2, mutualRecursion.odd(dt))
    }
    ...
    fun testBinaryTreeOddEvenNodesMutual() {
        val k = 20
        val bt = binaryTree(k)
        assertEquals(349525, mutualRecursion.even(bt))
        assertEquals(699050, mutualRecursion.odd(bt))
    }
    ...
    fun testEqualToAnythingClass() {
        ...
        val result = tailRecs.a.invoke(deepTree(100))
        assertEquals(1, tailRecs.nullCount)
    ...
    private val mutualRecursion = object {
